 ...
 *
 */
public class ML_DataExtraction extends DataExtraction {
    ...
    private String CUSTOM_TRANSFORM;
    ...
    public ML_DataExtraction(Logger LOGGER, Properties props) {
        this.LOGGER = LOGGER;
        this.props = props;
    }
    ...
    public void run() {
        try {
            ...
            Logger.getLogger(ML_DataExtraction.class.getName()).log(Level.SEVERE, null, ex);
    ...
    private void processMigration() throws InterruptedException, IOException {
        ...
        String logFilePath = MARKLOGIC_LOG_DIR + "Log_DataMigration_" + new SimpleDateFormat("YYYY-MM-dd_HH-mm-ss").format(new Date()) + ".txt";
        ...
        // CREATE OUTPUT FILE
        PrintStream out = new PrintStream(MARKLOGIC_OUTPUT_FILE_PATH);
        ...
        do {
            ...
            processPage = new ML_ProcessPage(documentPage, elements, out, DELIMITER, docMgr, MARKLOGIC_ROOT_NODE, CUSTOM_TRANSFORM);
    ...
    private void loadConstants(Properties props) throws IOException {
        MARKLOGIC_PASSWORD = props.getProperty("DB_PASSWORD");
        MARKLOGIC_USERNAME = props.getProperty("DB_USERNAME");
        MARKLOGIC_HOST = props.getProperty("DB_HOST");
        try {
            MARKLOGIC_PORT = Integer.parseInt(props.getProperty("DB_PORT"));
        } catch (NumberFormatException nex) {
            MARKLOGIC_PORT = 8000;
        }

        MARKLOGIC_NAME = props.getProperty("ML_DB_NAME");
        DELIMITER = props.getProperty("DELIMITER", "|");
        MARKLOGIC_ROOT_NODE = props.getProperty("ML_ROOT_NODE");

        try {
            MARKLOGIC_PAGE_SIZE = Integer.parseInt(props.getProperty("ML_PAGE_SIZE"));
        } catch (NumberFormatException nex) {
            MARKLOGIC_PAGE_SIZE = 10000;
        }

        MARKLOGIC_OUTPUT_FILE_PATH = props.getProperty("ML_OUTPUT_PATH").trim();
        MARKLOGIC_FILTERS_FILE = props.getProperty("ML_FILTERS_FILE_PATH").trim();
        MARKLOGIC_LOG_DIR = props.getProperty("ML_LOGS_DIR").trim();
        MARKLOGIC_ELEMENTS_FILE = props.getProperty("ML_ELEMENTS_FILE_PATH").trim();
    }
